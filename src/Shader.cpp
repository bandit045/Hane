...
std::string get_file_contents(const char* filename)
{
	...
	{
		std::cout << "Shader file " << filename << " can't be located, or does not exist!" << std::endl; // TODO: Throw an exception, and add spdif log
	}

	std::ifstream shaderFileToRead(filename, std::ios::binary);
	if (shaderFileToRead)
	{
		std::string contentInsideShaderFile;
		shaderFileToRead.seekg(0, std::ios::end);
		contentInsideShaderFile.resize(shaderFileToRead.tellg());
		shaderFileToRead.seekg(0, std::ios::beg);
		shaderFileToRead.read(&contentInsideShaderFile[0], contentInsideShaderFile.size());
		shaderFileToRead.close();
		return(contentInsideShaderFile);
	}
	throw(errno);
}
...
{
	//std::cout << "Constructing Shader Program for " << vertexFile << " and " << fragmentFile << "" << std::endl;
	...
	//std::cout << "Shader Program has constructed for " << vertexFile << " and " << fragmentFile << std::endl;
}
